...
class EditModeReloadStrategy(ReloadStrategy):
    ...
    def handle_reload(
        ...
        # Deletes
        for cid in deleted:
            ops.append(DeleteCell(cell_id=cid))
        ...
        for cd in cell_manager.cell_data():
            if cd.cell_id not in doc_ids:
                ...
                if cd.code != doc_cell.code:
                    ops.append(SetCode(cell_id=cd.cell_id, code=cd.code))
                if cd.name != doc_cell.name:
                    ops.append(SetName(cell_id=cd.cell_id, name=cd.name))
        ...
        # Reorder
        ops.append(ReorderCells(cell_ids=tuple(cell_ids)))
            ...
            session.notify(
                NotebookDocumentTransactionNotification(
                    transaction=Transaction(
                        ops=tuple(ops), source="file-watch"
                    )
                ),
